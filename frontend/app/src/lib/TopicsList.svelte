...
<ul>
  {#each topics as topic}
    <li>
      <div class="topic-name" on:click={() => toggle(topic.id)}>
        <span class="name {activeIndex === topic.id ? 'active' : ''}">{topic.name}</span>
        <span style="text-align: right">
        {#each {length: topic.hot} as _, i}
        ...
        {/each}
        <div class="count">{topic.posts_count} публикаций</div>
        </span>
      </div>
      <div class="content {activeIndex === topic.id ? 'open' : ''}">
        Регион
      </div>
    </li>
  {/each}
